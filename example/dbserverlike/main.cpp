...
class CDBServerLike : public IServer
{
public:
	virtual CProtocolParser* GetClientParser(uint16_t listener_id_)
	{
		switch (listener_id_) //match svr cfg listener_id
		{
		case 1:
		case 2:
			return &g_parser;
	...
	virtual void OnRecvClient(const void* data_, size_t data_len_, uint32_t session_id_, uint16_t listener_id_)
	{
		...
		{
			case SSP::ss_cmd_query_power_req:
			{
				...
				if(!req.fighterid()) // suppose 0 is invalid
 				{
					LOG_ERROR("power_req.fighterid invalid. session_id=%u", session_id_);
					rsp.set_errcode(1);
				}
				else
				{
					rsp.set_errcode(0);
					rsp.set_power(rand()%100000 + 1);
				}

				static char rsp_pack_buff[1024];
				SPackHead rsp_head;
				rsp_head.cmd = SSP::ss_cmd_query_power_rsp;
...
protected:
	...
	virtual void OnFini() { LOG_INFO("CDBServerLike::OnFini finish"); }
};
